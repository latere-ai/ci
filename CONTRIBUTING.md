...
Every sentence the pipelines emits or carries is written for one reader, and
the register follows the reader:

- User, a person or a coding harness: workflow step summaries, the release
  body a consumer repository publishes, `README.md`. Short and plain: what
  happened and what to do next, naming a command or a page, never a package,
  a function, a table, or a Kubernetes object.
- Contributor, someone changing the pipelines: specs, this file, package
  documentation, commit messages, source comments. Precise, in the project's
  own terms, with the reason a design is what it is.
- Developer, someone debugging a running system: step logs and the test
  scripts' failure output. Exact and complete: object, operation, observed
